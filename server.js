...
// Copiar cookies si existen (útil para Render)
...
async function fetchJSON(url, timeoutMs = 12000, extraHeaders = {}) {
  ...
  try {
    const r = await fetch(url, { signal: ctrl.signal, headers: { 'User-Agent': 'Mozilla/5.0', ...extraHeaders } });
...
async function jiosaavnSearch(q, limit = 15) {
  try {
    const data = await fetchJSON(`https://saavn.dev/api/search/songs?query=${encodeURIComponent(q)}&page=1&limit=${limit}`);
...
async function ytSearch(q, limit = 15) {
  for (let attempt = 0; attempt < INVIDIOUS.length; attempt++) {
    ...
    } catch (e) { console.warn(`[Invidious ${base}]`, e.message); }
  }
  return [];
}
...
async function invAudioUrl(videoId) {
  for (let attempt = 0; attempt < INVIDIOUS.length; attempt++) {
    ...
    } catch (e) { console.warn(`[Invidious audio ${base}]`, e.message); }
  }
...
async function ytGetAudioUrl(videoId) {
  const out = await ytdlpExec(`"https://www.youtube.com/watch?v=${videoId}" -f "bestaudio[ext=webm]/bestaudio[ext=m4a]/bestaudio" --get-url`, 45000);
  const url = out.split('\n')[0].trim();
...
function pipeAudio(audioUrl, res, req, extraArgs = []) {
  ...
  ff.stdout.pipe(res);
  ff.stderr.on('data', () => {}); // Silenciar logs de ffmpeg para no saturar consola
...
// ─── Routes ───────────────────────────────────────────────────────────────────
app.get('/', (_req, res) => res.json({ status: 'ok', message: 'Aletone API' }));
...
app.get('/api/stream/:videoId', async (req, res) => {
  ...
  // Cabeceras CRÍTICAS para streaming continuo en web y móvil sin cortes
  ...
  res.setHeader('Accept-Ranges', 'none'); // Previene que Safari intente buscar y detenga la carga
  ...
  try {
    ...
    console.warn('[Stream] Invidious failed, trying yt-dlp...');
  }
...
app.post('/api/download', async (req, res) => {
  ...
  if (fs.existsSync(filepath)) return res.json({ success: true, filename, url: `/downloads/${encodeURIComponent(filename)}` });
...
app.listen(PORT, () => console.log(`Aletone API activa en puerto ${PORT}`));